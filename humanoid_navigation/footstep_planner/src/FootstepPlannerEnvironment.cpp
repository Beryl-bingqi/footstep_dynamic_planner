// SVN $HeadURL: https://alufr-ros-pkg.googlecode.com/svn/trunk/humanoid_navigation/footstep_planner/src/FootstepPlannerEnvironment.cpp $
// SVN $Id: FootstepPlannerEnvironment.cpp 2242 2011-11-28 12:33:23Z hornunga@informatik.uni-freiburg.de $
...
namespace footstep_planner
{
    ...
    {
	ROS_INFO("Setup the initial start and goal foot for both left and right. ");
    ...
    {
	ROS_INFO("update the goal in FootstepPlannerEnvironment.");
    ...
    {
	ROS_INFO("Collision check for the planning state.");
	...
	ROS_INFO("transform the planning state to the foot center.");
    ...
    {
	ROS_INFO("Set the map for calculating path cost Heuristic. ");
        ivMapPtr.reset();
        ivMapPtr = map;
    ...
    {
	ROS_INFO("Reset the states of all the variables of the system.");
    ...
    {
	ROS_INFO("goal check for backward planning.");
    ...
    {
	ROS_INFO("goal check for forward planning.");
    ...
    {
	ROS_INFO("Calculate the parameterization of the footstep model.");
	...
	ROS_INFO("Compare the speed with other get_footstep function.");
    ...
    FootstepPlannerEnvironment::reachable(const PlanningState& from,
	                                      const PlanningState& to)
    {
	ROS_INFO("Check whether the x & y & theta are reachable.");
	        ...
	        std::vector<int>* pred_ids)
    {
		...
		for (; state_iter != changed_states.end(); state_iter++)
		{
			PlanningState s(*state_iter, ivCellSize, ivNumAngleBins,
			                ivHashTableSize);
		ROS_INFO("Check predecessor planning states.");
...
//					continue;
    ...
    {
    	succ_ids->clear();
		ROS_INFO("Check successors states.");
		std::vector<State>::const_iterator state_iter = changed_states.begin();
		for (; state_iter != changed_states.end(); state_iter++)
		{
			PlanningState s(*state_iter, ivCellSize, ivNumAngleBins,
...
//					continue;
    ...
    {
	ROS_INFO("Calculate the pres and cost for a targetStateID.");
        PredIDV->clear();
        CostV->clear();
    ...
    FootstepPlannerEnvironment::GetSuccs(int SourceStateID,
                                         ...
                                         std::vector<int> *CostV)
    {
	ROS_INFO("Calculate the succes and cost for a targetStateID.");
        SuccIDV->clear();
        CostV->clear();
